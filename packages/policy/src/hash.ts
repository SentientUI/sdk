...
// A plain array literal (not `new Uint32Array(...)`) so this module stays
// side-effect-free and bundlers tree-shake the table out of bundles that
// never call hashLayout (e.g. @sentientui/core's lean entry).
...
/** First 16 hex chars of SHA-256(input) — i.e. the first 8 digest bytes. */
function sha256HexPrefix16(input: string): string {
  const msg = new TextEncoder().encode(input);
  const len = msg.length;
  const bitLen = len * 8;
  const totalLen = (((len + 8) >> 6) + 1) << 6; // len + 0x80 + pad + 8-byte length, 64-aligned
  ...
  return toHex8(h0) + toHex8(h1);
}

export function hashLayout(order: string[]): string {
  return sha256HexPrefix16(order.join('|'));
}
