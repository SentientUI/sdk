...
- 1915f13: The on-site editor no longer takes its token from the URL. The dashboard now opens the site with a one-time, 60-second code in the URL fragment (`#sentient_editor_code=`), and the editor exchanges it once at `POST /v1/editor/exchange`, which checks the Origin against the project's allowed origins, for the short-lived editor token. That token is then held in memory and sessionStorage only. Before this, `?sentient_editor=<token>` sent a publish-capable token to the site's server and CDN logs, to analytics tools that record the page URL, and in the Referer of early subresource requests.

  The preview token and the editor code are now appended to any fragment the page already has (`#/route?sentient_preview_token=…`) instead of replacing it, and the site's own fragment is restored exactly when they are stripped. Previews of hash-routed pages now open on the right route.

  **Breaking:** a snippet or React SDK older than this release does not open the editor, cell preview, region refresh or style capture from a link made by the updated dashboard, and the updated SDKs ignore `?sentient_editor=`. CDN-tag snippet installs pick this up automatically. React installs need to upgrade `@sentientui/react`. The snippet also stops injecting `<meta name="referrer" content="no-referrer">` during editor sessions, because no secret travels in the URL any more.
...
- Updated dependencies [fc6d2cd]
- Updated dependencies [2cc598a]
- Updated dependencies [ca64b25]
- Updated dependencies [fc6d2cd]
  - @sentientui/core@0.36.0
...
- 9c06c39: `consentWatcher` is exported from `@sentientui/react` and available on the snippet as `window.SentientSnippet.consentWatcher`, so a site can gate its other consent-bound scripts (an ad pixel, a chat widget) on the same consent platform the SDK follows — accept and withdraw — without a separate dependency.
...
- Updated dependencies [6dcc0b9]
  - @sentientui/core@0.35.0
...
- Updated dependencies [0633527]
  - @sentientui/core@0.34.3
...
- Updated dependencies [14cfb6f]
  - @sentientui/core@0.34.2
...
- Updated dependencies [e125fe4]
  - @sentientui/core@0.34.1
...
- 46e7db2: Text versions of a slot now land inside the markup you wrote instead of replacing it. A generated version of a styled `<button>` keeps its element, classes and icon; only the words change. React: a text version only replaces a region with exactly one text; a region with several texts or unreachable text renders the original, with a dev-build warning.
- Updated dependencies [b79562c]
- Updated dependencies [46e7db2]
- Updated dependencies [0c6c86b]
  - @sentientui/core@0.34.0
...
- `<AdaptiveRoot registrySlotIds={[…]}>` decides generated-mode `<Adaptive id>` regions in the SSR round trip and preloads their served content (`slotConfig`/`palette`) into the provider, so first paint shows the published version instead of the original children. `loadAdaptiveDecision` already accepted the option; AdaptiveRoot never passed it. Scoped to the declared ids only — an unscoped registry decide would record a trial for every published slot on every page.

### Patch Changes

- Updated dependencies
  - @sentientui/policy@0.12.0
  - @sentientui/core@0.31.1
...
- Updated dependencies [f304caf]
- Updated dependencies [f304caf]
- Updated dependencies [036af5e]
- Updated dependencies [f304caf]
  - @sentientui/core@0.31.0
...
- Updated dependencies [68151cd]
  - @sentientui/core@0.30.0
...
- Updated dependencies [a3ce1dd]
  - @sentientui/core@0.29.0
...
- Updated dependencies [c9c1260]
  - @sentientui/core@0.28.0
...
- Updated dependencies [8ed4d0b]
  - @sentientui/core@0.16.10
