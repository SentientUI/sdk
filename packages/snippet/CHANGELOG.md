...
- 1106fb0: Preview mode (`?sentient_preview=`) forwards a draft-read credential to `/v1/explain`: `?sentient_preview_token=` (stripped from the URL on load) or the on-site editor's cached token. The API no longer returns unpublished slots to the public key alone; without a token, preview shows published slots only.
- b4830d2: The preview draft-read credential is now read from the URL fragment (`#sentient_preview_token=`) instead of the query string, and stripped from the fragment on load. A fragment is never sent to the site's server, CDN or analytics, nor in a Referer; the query form reached all of them before the snippet could strip it. The dashboard's preview links use the fragment; a query-string token is no longer read (preview then shows published slots only).
